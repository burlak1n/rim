...
type CreateContactRequest struct {
	...
	Phone     string `json:"phone" validate:"required,e164"` // e164 для базовой проверки формата телефона
	Email     string `json:"email" validate:"required,email"`
	Transport string `json:"transport,omitempty" validate:"omitempty,oneof=car license none"`
	Printer   string `json:"printer,omitempty" validate:"omitempty,oneof=color plain none"`
	Allergies string `json:"allergies,omitempty"`
	VK        string `json:"vk,omitempty" validate:"omitempty,url_encoded"`    // Проверка на URL-like строку
	Telegram  string `json:"telegram,omitempty" validate:"omitempty,alphanum"` // Базовая проверка username
...
type UpdateContactRequest struct {
	...
	Printer   *string `json:"printer,omitempty" validate:"omitempty,oneof=color plain none"`
	Allergies *string `json:"allergies,omitempty"`
	VK        *string `json:"vk,omitempty" validate:"omitempty,url_encoded"`
	Telegram  *string `json:"telegram,omitempty" validate:"omitempty,alphanum"`
	GroupIDs  *[]uint `json:"group_ids,omitempty"` // Для полной замены списка групп
}
